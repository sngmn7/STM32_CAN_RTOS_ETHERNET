...
/** @brief 성능 통계('SP' 5104) 1 Hz 송신 — 매 루프 호출 가능 */
void RearTelemetry_SendPerf(void);

/** @brief 진단 DTC('SD' 5105) 송신 — 상태 변화 시 + 1 Hz 하트비트 */
void RearTelemetry_SendDtc(uint32_t now);

extern volatile uint32_t rt_perf_tx_count;
extern volatile uint32_t rt_perf_tx_error_count;
extern volatile uint32_t rt_dtc_tx_count;
extern volatile uint32_t rt_dtc_tx_error_count;
