...
/* ================================================================== */
/* RTOS 전환 1단계 — 슈퍼루프를 기본 태스크로 그대로 옮긴다.          */
/*                                                                    */
/* 로직은 한 줄도 바꾸지 않는다. 바뀐 것은 두 가지뿐이다.             */
/*   - MX_LWIP_Process() 제거: NO_SYS=0 에서는 lwIP 가 자기 tcpip     */
/*     스레드로 스스로 돌기 때문에 폴링 함수가 존재하지 않는다.       */
/*   - while(1) -> 태스크의 for(;;)                                    */
/*                                                                    */
/* 태스크 분리는 이더넷 안정성을 먼저 검증한 뒤 2단계에서 한다.       */
/* ================================================================== */
void Rear_AppStep(void)
{
    uint32_t now = HAL_GetTick();
...
int main(void)
{
  ...
  /*
   * lwIP 를 쓰는 초기화(UDP 소켓 생성)는 여기서 하면 안 된다.
   *
   * NO_SYS=0 으로 바뀌면서 MX_LWIP_Init() 이 기본 태스크 안으로 옮겨갔다.
   * 이 시점에는 lwIP 가 아직 초기화되지 않아 udp_new() 가 실패하고,
   * Error_Handler() 가 __disable_irq() 를 걸어 시스템이 멈춘다.
   *
   * -> freertos.c 의 StartDefaultTask() 로 옮겼다.
   */
  ...
  /* 스케줄러가 도므로 여기에는 도달하지 않는다. 루프 본문은
     Rear_AppStep() 으로 옮겨 기본 태스크가 호출한다. */
  while (1)
  {
    /* USER CODE END WHILE */
