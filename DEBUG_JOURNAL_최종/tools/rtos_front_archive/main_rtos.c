...
/* 우선순위와 기동 방식                                               */
/*                                                                    */
/*   CTRL(3)  알림 기반. 지령이 바뀌면 NET 이 즉시 깨운다.            */
/*            깨울 일이 없어도 1 ms 마다 한 번은 돈다(하트비트).       */
/*   NET(1)   최저 우선순위 배경 폴링. 블로킹하지 않는다.             */
/*                                                                    */
/* 처음에는 둘 다 1 ms 주기 폴링으로 만들었는데, lwIP 가 인터럽트 없이 */
/* 완전 폴링 구조(ethernetif_input) 라서 폴링 빈도가 곧 지연이 된다.   */
/* 실측으로 ACK RTT p99 가 0.663 -> 1.856 ms 로 늘었다.                */
/*                                                                    */
/* 그래서 NET 을 최저 우선순위 무한 폴링으로 돌린다. CTRL 이 준비되면  */
/* 언제든 선점하므로 제어 결정성은 유지되고, 남는 시간은 전부 lwIP 가  */
/* 쓴다. 베어메탈의 폴링 속도를 되찾으면서 우선순위 구조는 남는다.     */
/*                                                                    */
/* lwIP 는 NO_SYS=1 을 유지한다. 따라서 **NET 태스크만** lwIP 를       */
/* 만져야 한다. 텔레메트리·진단 송신도 전부 NET 에 둔 이유다.          */
/* ================================================================== */
#define TASK_CTRL_PRIO        3U
#define TASK_NET_PRIO         1U
...
/**
 * @brief 하행 지령이 바뀌었으면 CTRL 태스크를 즉시 깨운다.
 *
 * 지령은 UDP 로 들어오므로 NET 태스크가 가장 먼저 안다. 주기를 기다리지
 * 않고 알려주면 CTRL 이 곧바로 CAN 으로 내보낼 수 있다.
 */
static void App_NotifyCtrlIfCommandChanged(void)
{
    static uint16_t snapshot[FC_SIG_COUNT];
    ...
    if (primed == 0U)
    {
        primed = 1U;      /* 첫 바퀴는 초기값 채우기만 */
        return;
    }
    if ((changed != 0U) && (h_task_ctrl != NULL))
    {
        (void)xTaskNotifyGive(h_task_ctrl);
    }
}
...
static void Task_Net(void *arg)
{
    ...
    for (;;)
    {
        ...
        App_NotifyCtrlIfCommandChanged();
        /* 블로킹하지 않는다 — 최저 우선순위라 상위 태스크가 언제든 선점한다. */
    }
}
