...
#define LEFT_LED_CMD_ID      0x320U
#define RIGHT_LED_CMD_ID     0x321U
#define BRAKE_LED_CMD_ID     0x322U
...
#define LAMP_COMMAND_TIMEOUT_MS  500U
...
static uint32_t led_left_last_rx_tick = 0U;
static uint32_t led_right_last_rx_tick = 0U;
static uint32_t led_brake_last_rx_tick = 0U;
...
static void IMU_PollAndSend(void)
{
  ...
  if (len >= 22 && rx[2] == 3 && rx[4] == 0xFB && rx[9] == 0x05)
  {
    ...
    /*
     * BNO08x SHTP 입력 리포트 배치 (채널 3)
     *   rx[0..3]   SHTP 헤더 (len L/H, channel, seq)
     *   rx[4]      0xFB  base timestamp reference
     *   rx[5..8]   timestamp delta (u32)
     *   rx[9]      Report ID (0x05 = Rotation Vector)
     *   rx[10]     Sequence
     *   rx[11]     Status
     *   rx[12]     Delay
     *   rx[13..14] quat_i    (int16 LE)   ← 쿼터니언 시작
     *   rx[15..16] quat_j
     *   rx[17..18] quat_k
     *   rx[19..20] quat_real
     *   rx[21..22] Accuracy
     *
     * 이전 코드는 rx[14..21] 을 실어 1바이트 밀려 있었다.
     * 각 int16 이 "앞 성분 상위바이트 + 뒤 성분 하위바이트"로 조합되어
     * 값이 요동쳤다.
     */
    ImuTxData[0] = rx[13]; ImuTxData[1] = rx[14];   /* quat_i    */
    ImuTxData[2] = rx[15]; ImuTxData[3] = rx[16];   /* quat_j    */
    ImuTxData[4] = rx[17]; ImuTxData[5] = rx[18];   /* quat_k    */
    ImuTxData[6] = rx[19]; ImuTxData[7] = rx[20];   /* quat_real */
...
/*
 * Rear H723에서 분리된 CAN ID(0x320/0x321/0x322)를 수신한다.
 * Front에 실제 적용된 좌/우 LED 상태를 그대로 미러링하며,
 * Rear F446RE에서는 별도의 점멸 위상을 만들지 않는다.
 */
...
static void LED_TurnSignalUpdate(void)
{
  ...
  if ((uint32_t)(now - led_left_last_rx_tick) > LAMP_COMMAND_TIMEOUT_MS)
  {
    led_left_cmd = 0U;
  }
  if ((uint32_t)(now - led_right_last_rx_tick) > LAMP_COMMAND_TIMEOUT_MS)
  {
    led_right_cmd = 0U;
  }
  if ((uint32_t)(now - led_brake_last_rx_tick) > LAMP_COMMAND_TIMEOUT_MS)
  {
    led_brake_cmd = 0U;
  }

  /* Front PE7/PE8 states are mirrored directly; no independent rear blink phase. */
  LED_Apply(led_left_cmd, led_right_cmd, led_brake_cmd);
}
...
int main(void)
{
  ...
  led_left_last_rx_tick = HAL_GetTick();
  led_right_last_rx_tick = HAL_GetTick();
  led_brake_last_rx_tick = HAL_GetTick();
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
  ...
  /* ===== Rear lamp commands: one signal per CAN ID ===== */
  if ((RxHeader.StdId == LEFT_LED_CMD_ID) && (RxHeader.DLC == 1U))
  {
    led_left_cmd = (RxData[0] != 0U) ? 1U : 0U;
    led_left_last_rx_tick = HAL_GetTick();
    led_cmd_rx_count++;
    return;
  }
  if ((RxHeader.StdId == RIGHT_LED_CMD_ID) && (RxHeader.DLC == 1U))
  {
    led_right_cmd = (RxData[0] != 0U) ? 1U : 0U;
    led_right_last_rx_tick = HAL_GetTick();
    led_cmd_rx_count++;
    return;
  }
  if ((RxHeader.StdId == BRAKE_LED_CMD_ID) && (RxHeader.DLC == 1U))
  {
    led_brake_cmd = (RxData[0] != 0U) ? 1U : 0U;
    led_brake_last_rx_tick = HAL_GetTick();
    led_cmd_rx_count++;
    return;
  }
